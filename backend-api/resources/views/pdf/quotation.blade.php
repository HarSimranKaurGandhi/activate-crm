  ...
  <div class="page">
    ...
    <div class="content">
      ...
      @elseif($company['letterhead_type'] === 'pdf' && $company['letterhead_src'])
        <div class="letterhead"><embed src="{{ $company['letterhead_src'] }}" type="application/pdf"></div>
      @endif
      <div class="ribbon">
        <div class="ribbon-brand">{{ $company['company_name'] ?: 'Quotation' }}</div>
      ...
      <div class="section">
        <div class="top-grid">
          ...
          <div class="meta-card">
            <div class="meta-row"><div class="meta-icon">No</div><div class="meta-label">Quote No.</div><div class="meta-value">{{ $quotation['number'] }}</div></div>
            <div class="meta-row"><div class="meta-icon">Dt</div><div class="meta-label">Date</div><div class="meta-value">{{ $quotation['quote_date_label'] }}</div></div>
            <div class="meta-row"><div class="meta-icon">St</div><div class="meta-label">Status</div><div class="meta-value"><span class="status-badge status-{{ $quotation['status_key'] }}">{{ $quotation['status_label'] }}</span></div></div>
          </div>
        </div>
          ...
          <table class="items-table">
            <thead>
              <tr>
                ...
                @if($quotation['show_discount']) <th class="center" style="width:70px;">Disc%</th> @endif
            ...
            <tbody>
              @foreach($quotation['items'] as $index => $item)
                <tr>
                  ...
                  <td class="specs">{!! $item['specifications_html'] !!}</td>
                  <td class="right amount-strong">{{ $item['edited_price_label'] }}</td>
                  <td class="center amount-strong">{{ $item['quantity_label'] }}</td>
                  @if($quotation['show_discount']) <td class="center">{{ $item['discount_percent_label'] }}</td> @endif
                  <td class="right"><div>{{ $item['gst_percent_label'] }}</div><div class="gst-sub">{{ $item['tax_amount_label'] }}</div></td>
                  <td class="right amount-strong">{{ $item['line_total_label'] }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
        ...
        <div class="totals">
          <div class="totals-row muted"><span>Sub Total</span><span>{{ $quotation['subtotal_label'] }}</span></div>
          @foreach($quotation['adjustments'] as $adjustment)<div class="totals-row"><span>{{ $adjustment['name'] }}</span><span>{{ $adjustment['amount_label'] }}</span></div>@endforeach
          @if(!$quotation['gst_inclusive'] && $quotation['tax_amount'] > 0)
            <div class="totals-row"><span>Total Before Tax</span><span>{{ $quotation['before_tax_total_label'] }}</span></div>
            <div class="totals-row"><span>GST</span><span>{{ $quotation['tax_amount_label'] }}</span></div>
          @endif
          <div class="totals-row grand-total"><span class="grand-label">Grand Total</span><span class="grand-value">{{ $quotation['grand_total_label'] }}</span></div>
        </div>
        <div class="bottom-grid">
          @if(count($quotation['terms']) > 0)
            <div class="card">
              <div class="card-header">Terms and Conditions</div>
              <div class="card-body">@foreach($quotation['terms'] as $index => $term)<div class="term-row"><div class="term-index">{{ $index + 1 }}</div><div>{{ $term['content'] }}</div></div>@endforeach</div>
            </div>
          @endif
          <div class="card">
            <div class="card-header">Company Details</div>
            <div class="card-body">@foreach($company['details'] as $detail)<div class="detail-row"><div class="detail-label">{{ $detail['label'] }}</div><div class="detail-colon">:</div><div class="detail-value">{{ $detail['value'] }}</div></div>@endforeach</div>
          </div>
        </div>
        <div class="footer">
